...
function testStaticBoth(
  name: string,
  fn: (
    Sandbox: CompatSandboxClass,
    Snapshot: CompatSnapshotClass,
  ) => Promise<void>,
) {
  test(`[mock] ${name}`, () => fn(MockSandbox, MockSnapshot));
  (RUN_INTEGRATION ? test : test.skip)(
    `[real] ${name}`,
    { timeout: STATIC_API_REAL_TIMEOUT_MS },
    () =>
      fn(
...
function describeBoth(
  ...
  realDescribe(`[real] ${name}`, () => {
    ...
    beforeAll(async () => {
      sandbox =
        await (RealSandbox.create() as unknown as Promise<CompatSandbox>);
    }, REAL_TIMEOUT_MS);
    afterAll(async () => {
      await sandbox?.stop();
    }, REAL_TIMEOUT_MS);
    fn({
      sandbox: () => sandbox,
      Sandbox: RealSandbox as unknown as CompatSandboxClass,
    });
  });
}
...
describe("sandbox compat", () => {
  ...
  describeBoth("file operations", ({ sandbox }) => {
    test("writeFiles + cat roundtrip", async () => {
      await sandbox().writeFiles([
        {
          path: "/tmp/compat-roundtrip.txt",
          content: Buffer.from("roundtrip"),
        },
      ]);
      const result = await sandbox().runCommand("cat", [
        "/tmp/compat-roundtrip.txt",
      ]);
      expect(result.exitCode).toBe(0);
    ...
    test("writeFiles preserves binary content", async () => {
      ...
      expect(
        await sandbox().readFileToBuffer({ path: "/tmp/compat-binary.bin" }),
      ).toEqual(content);
    });
  ...
  testBoth(
    "FileSystem facade preserves node-style behavior",
    async (sandbox) => {
      ...
      expect(
        (await sandbox.fs.lstat(`${root}/link.bin`)).isSymbolicLink(),
      ).toBe(true);
      expect(await sandbox.fs.realpath(`${root}/link.bin`)).toBe(
        `${root}/data.bin`,
      );

      await sandbox.fs.chmod(`${root}/data.bin`, "640");
      expect((await sandbox.fs.stat(`${root}/data.bin`)).mode & 0o777).toBe(
        0o640,
      );
      await sandbox.fs.truncate(`${root}/data.bin`, 2);
      expect(await sandbox.fs.readFile(`${root}/data.bin`)).toEqual(
        Buffer.from([1, 2]),
      );
  ...
  testStaticBoth(
    "getOrCreate initializes a named sandbox once",
    async (Sandbox) => {
      ...
      try {
        ...
        expect(await created.fs.readFile("/tmp/initialized.txt", "utf8")).toBe(
          "ready",
        );
      } finally {
  ...
  testStaticBoth(
    ...
    async (Sandbox, Snapshot) => {
      ...
      try {
        ...
        expect(
          (await sandbox.listSnapshots()).snapshots.map(({ id }) => id),
        ).toContain(snapshot.snapshotId);
        expect(
          (await Snapshot.list({ name })).snapshots.map(({ id }) => id),
        ).toContain(snapshot.snapshotId);
        const retrieved = await Snapshot.get({
  ...
  describeBoth("file utilities", ({ sandbox }) => {
    test("basename /tmp/foo/bar.txt", async () => {
      const result = await sandbox().runCommand("basename", [
        "/tmp/foo/bar.txt",
      ]);
      expect(result.exitCode).toBe(0);
    ...
    test("dirname /tmp/foo/bar.txt", async () => {
      const result = await sandbox().runCommand("dirname", [
        "/tmp/foo/bar.txt",
      ]);
      expect(result.exitCode).toBe(0);
  ...
  describeBoth("additional portability checks", ({ sandbox }) => {
    ...
    test("stat existing file", async () => {
      const result = await sandbox().runCommand("stat", [
        "/tmp/compat-link-target",
      ]);
      expect(result.exitCode).toBe(0);
  ...
  testBoth(
    "multi-user: home dir scoping and file operations",
    async (sandbox) => {
  ...
  testBoth(
    "multi-user: group creation and membership round-trip",
    async (sandbox) => {
      const username = `u${randomUUID().slice(0, 8).replace(/-/g, "")}`;
  ...
  testBoth(
    "extendTimeout extends the current session's timeout",
    async (sandbox) => {
  ...
  testBoth(
    "updateNetworkPolicy round-trips a mode-based policy",
    async (sandbox) => {
  ...
  testBoth("listSessions includes the current session", async (sandbox) => {
    ...
    const current = sessions.find(
      (s) => s.id === sandbox.currentSession().sessionId,
    );
  ...
  testBoth(
    "getCommand reattaches to a detached command by id",
    async (sandbox) => {
  ...
  testBoth(
    "openInteractive returns a websocket url and token",
    async (sandbox) => {
  ...
  testBoth(
    "mkDir creates a directory under a world-writable parent",
    async (sandbox) => {
  ...
  testBoth("sandbox-level file helpers round-trip", async (sandbox) => {
    ...
    await sandbox.writeFiles([
      { path: `${root}/nested/data.txt`, content: "sandbox-level" },
    ]);
    ...
    expect(
      await sandbox.readFileToBuffer({ path: `${root}/missing.txt` }),
    ).toBeNull();

    const localDir = path.join(
      tmpdir(),
      `sandbox-mock-download-${randomUUID().slice(0, 8)}`,
    );
  ...
  test("[mock] Snapshot.get() with an unknown id throws a 404", async () => {
    await expect(
      MockSnapshot.get({ snapshotId: "snap_missing" }),
  ...
  test("[mock] Sandbox.get() with unknown name throws", async () => {
    await expect(
      MockSandbox.get({ name: "nonexistent-name" }),
    ).rejects.toThrow();
  });
  ...
  (RUN_INTEGRATION ? test : test.skip)(
    ...
    async () => {
      await expect(
        RealSandbox.get({ name: "nonexistent-name" }),
      ).rejects.toThrow();
    },
  );
  ...
  // INCONSISTENCY: hostname command is unavailable in the real sandbox for this suite
  test.skip("INCONSISTENCY: hostname availability differs", () => {});

  // INCONSISTENCY: diff command is unavailable in the real sandbox for this suite
