...
import { formatBytes,
table, timeAgo } from "../util/output";
    ...
    console.log(
      table({
        ...
        columns: {
          ...
          EXPIRATION: { value: (s) => timeAgo(s.expiresAt) },
        ...
        return {
          ...
          async task() {
            ...
            if (snapshot.status !== 'created') {
              throw new Error(`Snapshot ${snapshotId} is in status "${snapshot.status}" and cannot be deleted.`)
            }
