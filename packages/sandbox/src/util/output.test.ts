...
describe("formatNextCursorHint", () => {
  ...
  it("uses a custom label when provided", () => {
    expect(
      ...
      "More ancestors: sandbox snapshots tree my-sandbox --direction desc --limit 10 --cursor snap_abc",
    );
  });

  it("defaults to 'More results' when label is omitted", () => {
    expect(
      formatNextCursorHint("sandbox list", { limit: 5 }, "cur"),
    ).toBe("More results: sandbox list --limit 5 --cursor cur");
  });
});
