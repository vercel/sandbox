...
describe("renderSnapshotTree", () => {
  ...
  test("renders only the current snapshot with a root marker when there are no ancestors or descendants", () => {
    ...
    expect(plain).toContain("expires: in 7 days");
  ...
  test("formats durations in days / hours / minutes", () => {
    ...
    expect(plain).toContain("expires: in 1 day");
    expect(plain).toContain("expires: in 3 days");
    expect(plain).toContain("expires: in 5 hours");
    expect(plain).toContain("expires: in 30 minutes");
  });
