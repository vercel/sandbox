...
**Breaking**: removed `sandbox create --sandbox-snapshot <name>` and `Snapshot.fromSandbox(name)`. Use `sandbox fork <name>` / `Sandbox.fork({ source: name })` instead. Raw snapshot IDs are still supported via `sandbox create --snapshot <id>` and `Sandbox.create({ source: { type: "snapshot", snapshotId } })`.
